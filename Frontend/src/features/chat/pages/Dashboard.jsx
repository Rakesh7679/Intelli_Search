...
const Dashboard = () => {
  ...
  return (
    <main className='min-h-screen w-full bg-[#07090f] p-3 text-white md:p-5'>
      <section className='flex h-[calc(100vh-1.5rem)] w-full gap-4 rounded-3xl border border-none p-1 md:h-[calc(100vh-2.5rem)] md:gap-6 md:p-1'>
        <aside className='hidden h-full w-72 shrink-0 rounded-3xl border  bg-[#080b12] p-4 md:flex md:flex-col'>
          <h1 className='mb-5 text-3xl font-semibold tracking-tight'>IntelliSearch</h1>

          <button
            type='button'
            onClick={createChat}
            className='mb-4 cursor-pointer rounded-xl border border-white/60 bg-white px-3 py-2 text-left text-base font-semibold text-[#080b12] transition hover:bg-white/90'
          >
            New Chat
          </button>
        ...
        <section className='flex h-full min-w-0 flex-1 flex-col gap-4'>
          ...
          <footer className='absolute bottom-8 left-1/2 w-1/2 -translate-x-1/2 rounded-3xl border border-white/60 bg-[#080b12] p-4 md:p-5'>
            <form onSubmit={handleSubmitMessage} className='flex flex-col gap-3 md:flex-row'>
              <input
                ...
                className='w-full rounded-2xl bg-transparent px-4 py-2.5 text-base text-white outline-none transition placeholder:text-white/45'
              />
              <button
                type='submit'
